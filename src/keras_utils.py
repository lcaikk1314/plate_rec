...
#keras加载和保存modle和weights：https://blog.csdn.net/jacke121/article/details/79245534?utm_source=blogxgwz7
...
def detect_lp(model,I,max_dim,net_step,out_size,threshold):
	...
	print("*****")
	print(I.shape[1::-1])#取出数据是(I.shape[1],I.shape[0])
	print(I.shape[:])
	print("*****")
	w,h = (np.array(I.shape[1::-1],dtype=float)*factor).astype(int).tolist()
	w += (w%net_step!=0)*(net_step - w%net_step)#保证w,h是net_step的整数倍
